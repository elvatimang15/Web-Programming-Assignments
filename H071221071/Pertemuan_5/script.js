...
window.onload = function() { //memuat website
...
function buildDeck() {
    ...
    for (let i = 0; i < types.length; i++) {
        for (let j = 0; j < values.length; j++) { //1x perulangan pada types dia melakukan sebanyak value
...
function startGame() {
    hidden = deck.pop(); //mngambil yg trkhir dan menghapusnya
    dealerSum += getValue(hidden);
    dealerAceCount += checkAce(hidden); //mengecek kartu as
    // console.log(hidden);
